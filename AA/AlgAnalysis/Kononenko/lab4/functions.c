...
matrix_t matr_mult_par(const matrix_t m1, const matrix_t m2, unsigned long long int *t, int num_threads)
{
    ...
    *t = tick();
    ...
    for (int i = 0; i < num_threads; ++i)
    {
        ...
        args[i].end_row = (i == num_threads - 1) ? res.n : (i + 1) * row_per_thread;
    }
    ...
    for (int i = arg->start_row; i < arg->end_row; i++)
        for (int j = 0; j < arg->m2.m; j++)
        {
            ...
            for (int k = 0; k < arg->m1.m / 2; k ++)
                arg->res.matr[i][j] += (arg->m1.matr[i][2*k+1] + arg->m2.matr[2*k][j]) *
                                  (arg->m1.matr[i][2*k] + arg->m2.matr[2*k+1][j]);
        }

    if (arg->m1.m  % 2 == 1)
        for (int i = arg->start_row; i < arg->m1.n; i++)
            for (int j = 0; j < arg->m2.m; j++)
                arg->res.matr[i][j] += arg->m1.matr[i][arg->m1.m - 1]* arg->m2.matr[arg->m2.n - 1][j];
}
