...
Matrix multThreadVinograd(const Matrix &a, const Matrix &b, int count)
{
    ...
    for(int i = 0; i < count; i++) {
        threads[i] = std::thread(&funcThread, std::ref(a), std::ref(b), std::ref(res), &rowFactor[0], &colFactor[0], i, count);
    }
    for(int i = 0; i < count; i++) {
